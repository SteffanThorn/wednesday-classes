...
export function getCapacityForSlot(day) {
  return SLOT_CAPACITY[day] ?? null;
}
...
// Formats a Date using its LOCAL calendar components (not toISOString, which converts to
// UTC and can shift the date by a day). getAvailableDatesByDay runs in the visitor's
// browser, so "local" here is the student's own timezone (e.g. NZ) - the calendar day
// they actually see and pick. Converting through UTC first was silently rolling early
// morning NZ bookings back to the previous UTC day, which then failed the server's
// (UTC-based) isAllowedClassDate check for every payment method.
...
export function inferDayFromClassName(className = '') {
  ...
  if (lower.includes('friday') && (lower.includes('6:00') || lower.includes('6pm') || lower.includes('18:00') || lower.includes('600'))) return 'friday-evening';
