...
const normalizeSenderEmail = (email) => String(email || '').trim().replace(/@innerlightyoga\.co\.nz$/i, '@innerlight.co.nz');

// Get appropriate sender based on environment
const getSender = () => {
  if (isLocal) {
    return normalizeSenderEmail(process.env.EMAIL_FROM_LOCAL || 'onboarding@resend.dev');
  }
  return normalizeSenderEmail(process.env.EMAIL_FROM_PRODUCTION || 'contact@innerlight.co.nz');
};
