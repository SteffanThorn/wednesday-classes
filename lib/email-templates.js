...
export function getBookingConfirmationEmail({ 
  ...
  const normalizedLanguage = String(preferredLanguage || 'en').toLowerCase().startsWith('zh') ? 'zh' : 'en';
  const classesRemainingLabel = normalizedLanguage === 'zh' ? '课程剩余' : 'Classes Remaining';
  const classesRemainingValue = remainingClasses !== null && remainingClasses !== undefined
    ? (normalizedLanguage === 'zh'
      ? `${remainingClasses}节`
      : `${remainingClasses} class${remainingClasses === 1 ? '' : 'es'}`)
    : (normalizedLanguage === 'zh' ? '暂无' : 'N/A');
    ...
    <tr>
      <td style="background-color: white; padding: 40px 30px; border-radius: 0 0 12px 12px;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f9fafb; border-radius: 12px; padding: 24px; margin-bottom: 30px;">
          ...
          <tr>
            <td style="padding: 16px 0 8px; border-top: 1px solid #e5e7eb;">
              <p style="margin: 0; color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">${classesRemainingLabel}</p>
              <p style="margin: 4px 0 0; color: #0ea5e9; font-size: 24px; font-weight: 600;">${classesRemainingValue}</p>
            </td>
          </tr>
          <tr>
...
Time: ${classTime}
Location: ${location}
${classesRemainingLabel}: ${classesRemainingValue}
