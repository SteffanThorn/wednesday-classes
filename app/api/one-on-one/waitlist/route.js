...
export async function POST(req) {
  try {
    ...
    const ipLimit = checkRateLimit(`waitlist:one-on-one:ip:${ip}`, { limit: 5, windowMs: 60 * 60 * 1000 });
    const emailLimit = checkRateLimit(`waitlist:one-on-one:email:${normalizedEmail}`, { limit: 2, windowMs: 60 * 60 * 1000 });
