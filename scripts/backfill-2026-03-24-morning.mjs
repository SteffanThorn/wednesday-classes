...
const targets = [
  { raw: 'Lindy', regex: /lindy\s*rell|lindy/i },
  { raw: 'glenys', regex: /glenys/i },
  { raw: 'ann', regex: /^ann$/i, exactOnly: true },
  { raw: 'yvette', regex: /yvette/i },
  { raw: 'deb', regex: /deb/i },
  { raw: 'linda macknight', regex: /linda\s*macknight/i },
  { raw: 'Linda Shannon', regex: /linda\s*shannon/i },
];
...
function matchStudent(target) {
  if (target.exactOnly) {
    const exact = allStudents.find((u) => /^ann$/i.test(String(u.name || '').trim()));
    if (exact) return exact;
    return (
      allStudents.find(
        (u) => /\bann\b/i.test(String(u.name || '')) || /\bann\b/i.test(String(u.email || ''))
      ) || null
    );
  }
  ...
  return (
    allStudents.find(
      (u) => target.regex.test(String(u.name || '')) || target.regex.test(String(u.email || ''))
    ) || null
  );
}
...
for (const target of targets) {
  const user = matchStudent(target);
